// lib/screens/which_is_farther_lobby_screen.dart
import 'package:flutter/material.dart';
import 'package:jidoapp/data/which_is_farther_data.dart';
import 'package:jidoapp/screens/which_is_farther_game_screen.dart';
...
class WhichIsFartherLobbyScreen extends StatefulWidget {
  final WhichIsFartherDifficulty difficulty;

  const WhichIsFartherLobbyScreen({super.key, required this.difficulty});

  @override
  State<WhichIsFartherLobbyScreen> createState() => _WhichIsFartherLobbyScreenState();
}

class _WhichIsFartherLobbyScreenState extends State<WhichIsFartherLobbyScreen> {
  final _service = WhichIsFartherScoreService();
  late Future<List<WhichIsFartherScoreRecord>> _recordsFuture;
  ...
  Future<void> _startGame() async {
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => WhichIsFartherGameScreen(difficulty: widget.difficulty)),
    );
    _reload();
  }
  ...
  Widget _buildHistory(Color accent) {
    return FutureBuilder<List<WhichIsFartherScoreRecord>>(
          ...
          children: [
            ...
            Expanded(
              child: ListView.separated(
                ...
                itemBuilder: (context, i) => _RecordTile(rank: i + 1, record: records[i]),
              ),
            ),
          ],
        );
...
class _RecordTile extends StatelessWidget {
  final int rank;
  final WhichIsFartherScoreRecord record;

  const _RecordTile({required this.rank, required this.record});
