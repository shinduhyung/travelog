// lib/screens/champions_league_teams_screen.dart
...
class ChampionsLeagueTeamsScreen extends StatelessWidget {
  const ChampionsLeagueTeamsScreen({super.key});

  static final List<Map<String, dynamic>> _clStadiums = [
    {'name': 'Old Trafford',          'iso': 'GB'},
    {'name': 'Camp Nou',              'iso': 'ES'},
    {'name': 'Santiago Bernabeu',     'iso': 'ES'},
    {'name': 'San Siro',              'iso': 'IT'},
    {'name': 'Allianz Arena',         'iso': 'DE'},
    {'name': 'Anfield',               'iso': 'GB'},
    {'name': 'Stamford Bridge',       'iso': 'GB'},
    {'name': 'Johan Cruijff Arena',   'iso': 'NL'},
    {'name': 'Estadio da Luz',        'iso': 'PT'},
    {'name': 'Estadio do Dragao',     'iso': 'PT'},
    {'name': 'Celtic Park',           'iso': 'GB'},
    {'name': 'De Kuip',               'iso': 'NL'},
    {'name': 'Stadion Rajko Mitic',   'iso': 'RS'},
    {'name': 'Orange Velodrome',      'iso': 'FR'},
  ];
        ...
        child: Column(
          ...
          children: [
            Container(color: Colors.white, child: Column(children: [
              Container(height: 4, color: const Color(0xFFE8854A)),
              Padding(
                padding: const EdgeInsets.fromLTRB(20, 16, 20, 16),
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Container(width: 52, height: 52,
                      decoration: BoxDecoration(color: Colors.white,
                          border: Border.all(color: const Color(0xFFE8854A), width: 2),
                          borderRadius: BorderRadius.circular(12)),
                      child: const Icon(Icons.emoji_events_outlined, color: Color(0xFFE8854A), size: 26)),
                  const SizedBox(width: 14),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text('Champions League', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                        color: Color(0xFFE8854A), letterSpacing: 0.6)),
                    const SizedBox(height: 3),
                    const Text('CL Stadiums', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w900,
                        color: Color(0xFF111827), letterSpacing: -0.5, height: 1.1)),
                    const SizedBox(height: 4),
                    Text('Historic UEFA Champions League grounds', style: TextStyle(fontSize: 12, color: Colors.grey[500])),
                  ])),
                  const SizedBox(width: 12),
                  Consumer<LandmarksProvider>(builder: (context, provider, _) {
                    final seen = _clStadiums.where((m) => provider.visitedLandmarks.contains(m['name'])).length;
                    final pct  = 14 > 0 ? seen / 14 : 0.0;
                    return Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
                      Row(mainAxisSize: MainAxisSize.min, children: [
                        Text('$seen', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900,
                            color: Color(0xFFE8854A), height: 1.0)),
                        Padding(padding: const EdgeInsets.only(left: 3, top: 6),
                            child: Text(' / 14', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600,
                                color: const Color(0xFFE8854A).withOpacity(0.45), height: 1.0))),
                      ]),
                      const SizedBox(height: 5),
                      SizedBox(width: 56, child: ClipRRect(borderRadius: BorderRadius.circular(3),
                          child: Stack(children: [
                            Container(height: 4, color: const Color(0xFFE8854A).withOpacity(0.12)),
                            FractionallySizedBox(widthFactor: pct,
                                child: Container(height: 4, color: const Color(0xFFE8854A))),
                          ]))),
                    ]);
                  }),
                ]),
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemCount: _clStadiums.length,
                itemBuilder: (context, index) {
                  final data      = _clStadiums[index];
                  ...
                  return GestureDetector(
                    onTap: () {
                      if (landmark != null) {
                        _showLandmarkDetailsModal(context, landmark, const Color(0xFFE8854A));
                      } else {
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(color: isVisited ? const Color(0xFFE8854A).withOpacity(0.5) : Colors.grey[200]!, width: isVisited ? 1.5 : 1),
                      ...
                      child: Row(children: [
                        ...
                        if (isVisited)
                          Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                              decoration: BoxDecoration(color: const Color(0xFFE8854A).withOpacity(0.12), borderRadius: BorderRadius.circular(12)),
                              child: Text('Visited',
                                  style: TextStyle(fontSize: 11, color: const Color(0xFFE8854A), fontWeight: FontWeight.w700)))
                        else
...
class _LandmarkVisitEditorCard extends StatefulWidget {
  ...
  @override State<_LandmarkVisitEditorCard> createState() => _LandmarkVisitEditorCardState();
}
