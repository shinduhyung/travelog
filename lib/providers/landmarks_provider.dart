...
// 3. 디버그 로그 및 Local Rank 매핑
      ...
      printTopUniversities ();
      printSpecificCityLandmarks(); // <--- 새로 추가된 함수 호출 부분
      await checkMissingFromCityRank();
  ...
  Future<void> checkMissingFromCityRank() async {
    try {
      final String response = await rootBundle.loadString('assets/city_rank.json');
      final Map<String, dynamic> cityRankData = json.decode(response);

      final Set<String> dbNames = _allLandmarks.map((l) => l.name.toLowerCase().trim()).toSet();
      List<String> missingLandmarks = [];

      cityRankData.forEach((city, landmarks) {
        if (landmarks is List) {
          for (var landmark in landmarks) {
            final String lmName = landmark.toString();
            if (!dbNames.contains(lmName.toLowerCase().trim())) {
              missingLandmarks.add('[$city] $lmName');
            }
          }
        }
      });

      print('\n[City Rank Missing Landmarks Check]');
      if (missingLandmarks.isEmpty) {
        print('결과: city_rank.json의 모든 관광지가 all_landmarks.json에 존재합니다.');
      } else {
        print('결과: 총 ${missingLandmarks.length}개의 관광지가 누락되었습니다:');
        for (var name in missingLandmarks) {
          print('- $name');
        }
      }
    } catch (e) {
      print('Error loading or checking city_rank.json: $e');
    }
  }
...
// visitDates 있는 항목을 _visitedLandmarks에 추가 (clear 없이 — 기존 set 유지)
    for (var landmark in _allLandmarks) {
...
// List of Target Cities Request
  void printSpecificCityLandmarks() {
    print('\n[Specific City Landmarks Check]');
    int printedCount = 0;

    final targetCities = [
      'Abu Dhabi', 'Accra', 'Adelaide', 'Ahmedabad', 'Almaty', 'Amman', 'Amsterdam',
      'Antwerp', 'Athens', 'Atlanta', 'Auckland', 'Austin', 'Baku', 'Baltimore',
      'Bangkok', 'Barcelona', 'Beijing', 'Beirut', 'Belgrade', 'Bengaluru', 'Berlin',
      'Bilbao', 'Birmingham', 'Bogotá', 'Bologna', 'Boston', 'Bratislava', 'Brisbane',
      'Bristol', 'Brussels', 'Bucharest', 'Budapest', 'Buenos Aires', 'Cairo', 'Calgary',
      'Cali', 'Cape Town', 'Caracas', 'Casablanca', 'Changchun', 'Changsha', 'Charlotte',
      'Chengdu', 'Chennai', 'Chicago', 'Chongqing', 'Cleveland', 'Colombo', 'Copenhagen',
      'Dallas', 'Dar es Salaam', 'Denver', 'Detroit', 'Dhaka', 'Doha', 'Dubai', 'Dublin',
      'Düsseldorf', 'Edinburgh', 'Frankfurt', 'Fuzhou', 'Geneva', 'George Town',
      'Guangzhou', 'Guatemala City', 'Haikou', 'Hamburg', 'Hangzhou', 'Hanoi', 'Harbin',
      'Hefei', 'Helsinki', 'Ho Chi Minh City', 'Hong Kong', 'Houston', 'Hyderabad',
      'Islamabad', 'Istanbul', 'Jakarta', 'Jeddah', 'Jinan', 'Johannesburg', 'Kampala',
      'Karachi', 'Katowice', 'Kuala Lumpur', 'Kunming', 'Kuwait City', 'Kyiv', 'Kyoto',
      'Labuan', 'Lagos', 'Lahore', 'Lima', 'Lisbon', 'Ljubljana', 'London', 'Los Angeles',
      'Luxembourg', 'Lyon', 'Madrid', 'Managua', 'Manama', 'Manchester', 'Manila',
      'Marseille', 'Melbourne', 'Mexico City', 'Miami', 'Milan', 'Minneapolis', 'Monterrey',
      'Montevideo', 'Montreal', 'Moscow', 'Mumbai', 'Munich', 'Muscat', 'Nairobi',
      'Nanchang', 'Nanjing', 'Nashville', 'New Delhi', 'New York City', 'Nicosia', 'Ningbo',
      'Osaka', 'Oslo', 'Panama City', 'Paris', 'Perth', 'Philadelphia', 'Phnom Penh',
      'Pittsburgh', 'Port Louis', 'Porto', 'Poznań', 'Prague', 'Pune', 'Qingdao', 'Quito',
      'Riga', 'Rio de Janeiro', 'Riyadh', 'Rome', 'Rotterdam', 'San Diego', 'San Francisco',
      'San Jose', 'San José', 'San Salvador', 'Santiago', 'Santo Domingo', 'São Paulo',
      'Seattle', 'Seoul', 'Shanghai', 'Shenyang', 'Shenzhen', 'Singapore', 'Sofia',
      'St. Louis', 'St. Petersburg', 'Stockholm', 'Stuttgart', 'Surabaya', 'Suzhou',
      'Sydney', 'Taipei', 'Taiyuan', 'Tampa', 'Tbilisi', 'Tegucigalpa', 'Tel Aviv', 'Tianjin',
      'Tirana', 'Tokyo', 'Toronto', 'Tunis', 'Turin', 'Valencia', 'Vancouver', 'Vienna',
      'Warsaw', 'Washington D.C.', 'Wellington', 'Wuhan', "Xi'an", 'Xiamen', 'Zagreb',
      'Zhengzhou', 'Zurich'
    ];

    for (var landmark in _allLandmarks) {
      if (landmark.city != null && targetCities.contains(landmark.city)) {
        print('City: ${landmark.city}, Landmark: ${landmark.name}');
        printedCount++;
      }
    }
    print('Total landmarks found for specific cities: $printedCount\n');
  }
}