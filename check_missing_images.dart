...
void main() {
  ...
  cityData.forEach((city, landmarks) {
    for (var landmark in landmarks) {
      final fileName = toSnake(landmark);
  ...
  final int missingCount = missingByCity.values.fold(0, (sum, list) => sum + list.length);
  final int existingCount = existingByCity.values.fold(0, (sum, list) => sum + list.length);
  final int orphanedCount = orphanedFiles.length;

  print('1. 필요 파일 : $missingCount개');
  print('2. 완료 파일 : $existingCount개');
  print('3. 정리 대상 : $orphanedCount개');

  print('\n\n--- 1. [필요 파일] (city_rank엔 있는데 사진 없음) ---');
